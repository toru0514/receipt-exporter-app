...
interface MicroCMSIncomeResponse {
  id: string;
  createdAt: string;
  updatedAt: string;
  date?: string;
...
function toIncome(item: MicroCMSIncomeResponse): Income {
  return {
    id: item.id,
    date: item.date ?? "",
...
export async function getIncomes(params?: {
  ...
  const response = await client.getList<MicroCMSIncomeResponse>({
    endpoint: "incomes",
    queries: {
      limit,
      offset,
  ...
  return {
    incomes: response.contents.map(toIncome),
    totalCount: response.totalCount,
  };
}
...
export async function getDistinctClients(): Promise<string[]> {
  ...
  const clients = response.contents
...
export async function createIncome(
  ...
  const response = await client.create<MicroCMSIncomeResponse>({
    endpoint: "incomes",
    content: {
      date: input.date,
    ...
    } as unknown as MicroCMSIncomeResponse,
  });
